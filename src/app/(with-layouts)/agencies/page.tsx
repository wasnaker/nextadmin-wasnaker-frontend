...
export default function AgenciesPage() {
  ...
  const { data: items = [], isPending } = useQuery({
    ...
    queryFn: async () => {
      const res = await api<{ data: Agency[] }>('/api/v1/agencies');
  ...
  const columns: SmallTableColumn<Agency>[] = [
    {
      key: 'id',
      label: 'ID',
      primary: true,
  ...
  async function onSave() {
    ...
    try {
      ...
      const res = await api(
        editing ? `/api/v1/agencies/${editing.id}` : '/api/v1/agencies',
        {
      ...
      setEditing(null);
      await qc.invalidateQueries({ queryKey: ['spine', 'agencies'] });
      const savedId = (res.data as Agency).id;
      selectItem(savedId);
      setRefreshKey((k) => k + 1);
    } catch {
  ...
  async function onDelete(item: Agency) {
    if (!window.confirm(`Hapus agency ${item.name} (${item.code})?`)) return;
    const res = await api(`/api/v1/agencies/${item.id}`, {
      method: 'DELETE',
    });
    if (!res.ok) {
    ...
    <div className='space-y-6'>
      <div className='flex flex-wrap items-start justify-between gap-4'>
        <div>
          ...
          <p className='mt-1 text-sm text-text-secondary'>
            Data agency + unit. Unit per agency di tab Units.
          </p>
        </div>
        {canCreate && <Button onClick={openCreate}>Add Agency</Button>}
      </div>
      ...
      {open && (
        <Dialog isOpen={open} onOpenChange={setOpen}>
          <DialogHeader>
            <DialogTitle>
              {editing ? `Edit Agency #${editing.id}` : 'Add Agency'}
            </DialogTitle>
          </DialogHeader>
          <DialogBody className='space-y-3'>
            ...
            <div>
              ...
              <Input
                ...
                placeholder='PT Agency Nusantara'
            ...
            <div className='grid grid-cols-2 gap-3'>
              <div>
                <FieldLabel htmlFor='f-province'>Provinsi</FieldLabel>
                <Select
              ...
              <div>
                <FieldLabel htmlFor='f-regency'>Kabupaten/Kota</FieldLabel>
                <Select
